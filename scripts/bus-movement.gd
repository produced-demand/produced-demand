...
func _process(delta: float) -> void:
	...
	# if there is a station in range
		...
		# station visited, out of range
		if visited:
			if distance_to_station > 80:
		...
		# approaching station (visited == false)
		else:
			# arrived
			if distance_to_station < (parent.get_progress() + change / 2):
				var marker = Sprite2D.new()
				marker.texture = load("res://assets/station.svg")
				marker.global_position = global_position	
